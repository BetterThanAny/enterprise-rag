...
2. Repository queries include tenant, knowledge-base, current-version, and ACL predicates before
   lexical or vector ranking. Unauthorized chunks never enter the candidate set.
...
- No OCR/layout model, Kubernetes, multi-region failover, or hosted production deployment.
